...
import * as data from './../json/messages-for-dead.json';
console.log(data);
...
class Contestant {
    ...
    render(context) {
        ...
        if(!debug) {
            ...
            context.save();
            context.translate(spriteWidthImage, 1)
            ...
            context.drawImage(sprites, 
                ...
                (this.direction == -1)? spriteWidthImage - tmpSpriteFrame.width/2 - this.position.x : this.position.x - spriteWidthImage, // - 608 
                this.position.y, 
...
function render(context) {
    ...
    tmpContestants = [...contestants].sort(function(a, b) {
...
function resurrect() {
    ...
    let tmpReverseLog = log.slice().reverse()
...
function eliminate() {
    ...
    for (let i = 0; i < eliminationsAmount; i++) {
        ...
        contestantIndex = contestants.map(function(contestant) { return contestant.name; }).indexOf(name)
        ...
        if(liveContestants.length == 1) {
            contestantIndex = contestants.map(function(contestant) { return contestant.name; }).indexOf(liveContestants[0].name)
...
function updateContestantsList() {
    tmpContestants = [...contestants].sort(function(a, b) {return b.health - a.health});
...
function loadContestants() {
    ...
    contestantsInputArray.forEach(contestantInput => {
        ...
        contestantIndex = contestants.map(function(contestant) { return contestant.name; }).indexOf(contestantInput.trim())
